//go:build !codegen

// Code generated by github.com/alanshaw/dag-json-gen. DO NOT EDIT.

package datamodel

import (
	"errors"
	"fmt"
	"io"
	"math"
	"sort"

	jsg "github.com/alanshaw/dag-json-gen"
	datamodel "github.com/fil-forge/ucantone/ipld/datamodel"
	cid "github.com/ipfs/go-cid"
)

var _ = cid.Undef
var _ = math.E
var _ = sort.Sort
var _ = errors.Is

func (t *ResultModel) MarshalDagJSON(w io.Writer) error {
	jw := jsg.NewDagJsonWriter(w)
	if t == nil {
		err := jw.WriteNull()
		return err
	}
	if err := jw.WriteObjectOpen(); err != nil {
		return err
	}
	written := false

	// t.Err (datamodel.Raw) (struct)
	if len("Err") > 8192 {
		return fmt.Errorf("string in field \"Err\" was too long")
	}
	if err := jw.WriteString(string("Err")); err != nil {
		return fmt.Errorf("writing string for field \"Err\": %w", err)
	}
	if err := jw.WriteObjectColon(); err != nil {
		return err
	}
	if err := t.Err.MarshalDagJSON(jw); err != nil {
		return fmt.Errorf("marshaling field t.Err: %w", err)
	}
	written = true
	if written {
		if err := jw.WriteComma(); err != nil {
			return err
		}
	}

	// t.Ok (datamodel.Raw) (struct)
	if len("Ok") > 8192 {
		return fmt.Errorf("string in field \"Ok\" was too long")
	}
	if err := jw.WriteString(string("Ok")); err != nil {
		return fmt.Errorf("writing string for field \"Ok\": %w", err)
	}
	if err := jw.WriteObjectColon(); err != nil {
		return err
	}
	if err := t.Ok.MarshalDagJSON(jw); err != nil {
		return fmt.Errorf("marshaling field t.Ok: %w", err)
	}
	if err := jw.WriteObjectClose(); err != nil {
		return err
	}
	return nil
}
func (t *ResultModel) UnmarshalDagJSON(r io.Reader) (err error) {
	*t = ResultModel{}

	jr := jsg.NewDagJsonReader(r)
	defer func() {
		if err == io.EOF {
			err = io.ErrUnexpectedEOF
		}
	}()
	if err := jr.ReadObjectOpen(); err != nil {
		return fmt.Errorf("reading object open for ResultModel: %w", err)
	}
	close, err := jr.PeekObjectClose()
	if err != nil {
		return fmt.Errorf("peeking object close for ResultModel: %w", err)
	}
	if close {
		if err := jr.ReadObjectClose(); err != nil {
			return fmt.Errorf("reading object close for ResultModel: %w", err)
		}
	} else {
		for i := uint64(0); i < 8192; i++ {
			name, err := jr.ReadString(8192)
			if err != nil {
				if errors.Is(err, jsg.ErrLimitExceeded) {
					return fmt.Errorf("reading string for field ResultModel: string too large")
				}
				return fmt.Errorf("reading string for field ResultModel: %w", err)
			}
			if err := jr.ReadObjectColon(); err != nil {
				return fmt.Errorf("reading object colon for field ResultModel: %w", err)
			}
			switch name {

			// t.Err (datamodel.Raw) (struct)
			case "Err":

				{
					null, err := jr.PeekNull()
					if err != nil {
						return fmt.Errorf("peeking null for field t.Err: %w", err)
					}
					if null {
						if err := jr.ReadNull(); err != nil {
							return fmt.Errorf("reading null for field t.Err: %w", err)
						}
					} else {
						t.Err = new(datamodel.Raw)
						if err := t.Err.UnmarshalDagJSON(jr); err != nil {
							return fmt.Errorf("unmarshaling t.Err pointer: %w", err)
						}
					}
				}

				// t.Ok (datamodel.Raw) (struct)
			case "Ok":

				{
					null, err := jr.PeekNull()
					if err != nil {
						return fmt.Errorf("peeking null for field t.Ok: %w", err)
					}
					if null {
						if err := jr.ReadNull(); err != nil {
							return fmt.Errorf("reading null for field t.Ok: %w", err)
						}
					} else {
						t.Ok = new(datamodel.Raw)
						if err := t.Ok.UnmarshalDagJSON(jr); err != nil {
							return fmt.Errorf("unmarshaling t.Ok pointer: %w", err)
						}
					}
				}

			default:
				// Field doesn't exist on this type, so ignore it
				if err := jr.DiscardType(); err != nil {
					return fmt.Errorf("ignoring field %s for ResultModel: %w", name, err)
				}
			}

			close, err := jr.ReadObjectCloseOrComma()
			if err != nil {
				return fmt.Errorf("reading object close or comma for field ResultModel: %w", err)
			}
			if close {
				break
			}
			if i == 8192-1 {
				return fmt.Errorf("map too large for ResultModel")
			}
		}
	}

	return nil
}
