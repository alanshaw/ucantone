...
func (t *EnvelopeModel) MarshalDagJSON(w io.Writer) error {
	...
	if err := jw.WriteArrayOpen(); err != nil {
		return fmt.Errorf("writing array open for field EnvelopeModel: %w", err)
	}
	...
	if err := jw.WriteComma(); err != nil {
		return fmt.Errorf("writing comma for field SigPayload: %w", err)
	}

	// t.SigPayload (datamodel.Raw) (struct)
	if err := t.SigPayload.MarshalDagJSON(jw); err != nil {
		return fmt.Errorf("marshaling field t.SigPayload: %w", err)
	}
	if err := jw.WriteArrayClose(); err != nil {
		return fmt.Errorf("writing array close for field EnvelopeModel: %w", err)
	}
	return nil
}
...
func (t *EnvelopeModel) UnmarshalDagJSON(r io.Reader) (err error) {
	*t = EnvelopeModel{}
	...
	if err := jr.ReadArrayOpen(); err != nil {
		return fmt.Errorf("reading array open for field EnvelopeModel: %w", err)
	}
	...
	if close {
...

		{
			...
			if err != nil {
				return fmt.Errorf("reading array close or comma for field EnvelopeModel: %w", err)
			}
			if close {
